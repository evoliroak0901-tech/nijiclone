        ...
        <div className="flex flex-col items-center justify-center h-full p-6 text-gray-200 overflow-y-auto">
            <div className="max-w-md w-full bg-[#252630] rounded-xl p-6 shadow-xl border border-[#3a3a45]">
                ...
                <p className="text-sm text-gray-400 mb-6 leading-relaxed">
                    画像生成を利用するには、Google Gemini APIキーが必要です。
                    このアプリは現在「BYOK (Bring Your Own Key)」方式で動作しており、ご自身で取得したキーを設定する必要があります。
                ...
                {savedKey ? (
                    ...
                    <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-4 mb-6 flex flex-col gap-2 text-yellow-400">
                        <div className="flex items-center gap-2">
                            <AlertTriangle size={20} />
                            <span className="text-sm font-bold">APIキーが設定されていません</span>
                        </div>
                        <p className="text-[10px] text-gray-400">
                            生成ボタンを押す前に、以下のフォームからキーを登録してください。
                        </p>
                    </div>
                )}
                ...
                {!savedKey && (
                    <div className="space-y-4">
                        ...
                        <button
                            ...
                            className={`w-full py-3 rounded-lg font-medium flex items-center justify-center gap-2 transition-all ${apiKey.trim()
            ...
            {status !== 'idle' && (
                <div className={`fixed bottom-20 left-1/2 transform -translate-x-1/2 px-6 py-3 rounded-full shadow-2xl z-50 transition-all duration-300 ${status === 'saved' ? 'bg-green-500 text-white' : 'bg-red-500 text-white'
                    }`}>
